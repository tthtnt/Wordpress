...
class Huge_it_catalog_Widget extends WP_Widget {
	...
	public function form( $instance ) {
		...
		<p>
				...
				<label for="<?php echo $this->get_field_id('catalog_id'); ?>"><?php echo __('Select Catalog:'); ?></label> 
        ...
        if($_POST["post"] == "productratingfromuser"){
            ...
            elseif($_POST["post"] == "load_more_elements_into_catalog"){
				...
				if($type == 'search') {
					...
					:"SELECT * FROM `".$wpdb->prefix."huge_it_catalog_products` WHERE (`catalog_id`='".$catalog_id."' AND (`name` LIKE '%".$test."%'".$like.")) order by ordering ASC";
				}
				else if($type == 'load') {
					...
					$query = $wpdb->prepare('SELECT * FROM '.$wpdb->prefix.'huge_it_catalog_products WHERE catalog_id =%d order by ordering ASC', $catalog_id);
				}
                ...
                switch ($view){
                    case 0:
                        $group_key = sanitize_text_field($_POST['elems_count']);
                        $moreImages = "";
...
//                             
                ...
                foreach($moreImagesInArray as $key=>$row)
                {
                    ...
                    $thumbs_li = "";
                    foreach($imgurl as $key=>$img){ if($img != "" && $img != ";"){ $thumbs_li .="<li><a href='".$row->sl_url."' class=''><img src='".$img."'></a></li>"; }  }
                    $next_popup  = $group_key+2;
                    $prev_popup = $group_key;
			...
			<div style='display: inline-block; float: left;'>
                                <div class='left-change' ><a href='#". $prev_popup  ."' data-popupid='#". $row->id. "'><</a></div>
                                <div class='right-change' ><a href='#". $next_popup  ."' data-popupid='#". $row->id ."'>></a></div>
            </div>
			...
			<div class='popup-wrapper_".$catalog_id."'>
                            <div class='image-block_".$catalog_id."'>";
                                if($row->image_url != ';'){
                                   $morePopups .= "<img id='wd-cl-img".$key."' src='". $imgurl[0] ."' />";
                                }else{
                    ...
                    case 3:
                        ...
                        foreach($moreImagesInArray as $key=>$row)
                        {
                                ...
                                $moreImages .= "<div class='left-block_".$catalog_id."'>
                                                <div class='main-image-block main-image-block_".$catalog_id." for_zoom'>";
...
('ht_catalog_zoom_window_border_size', 'Zoom Window Border Size', 'Zoom Window Size', '0'),
...
//    $wpdb->query($sql_huge_it_catalog_related_params);
    ...
    //wp_die("get ".$wpdb->get_var("select count(*) from " . $wpdb->prefix . "huge_it_catalogs")."get");
...
//    }
    ...
    ('ht_single_product_asc_seller_popup_background_2', 'ht_single_product_asc_seller_popup_background_2', 'ffffff'),
    ('ht_single_product_your_mail_text', 'Your E-mail', 'Your E-mail'),
    ('ht_single_product_your_phone_text', 'Your Phone', 'Your Phone'),
    ('ht_single_product_your_message_text', 'Your Message', 'Your Message'),
    ('ht_single_product_asc_seller_popup_button_text', 'ht_single_product_asc_seller_popup_button_text', 'Submit'),
    ('ht_single_product_asc_seller_popup_button_text_size', 'ht_single_product_asc_seller_popup_button_text_size', '19'),
    ('ht_single_product_asc_seller_popup_button_text_color', 'ht_single_product_asc_seller_popup_button_text_color', 'fff'),
    ('ht_single_product_asc_seller_popup_button_background_color', 'ht_single_product_asc_seller_popup_button_background_color', 'E22828'),
    ('ht_single_product_asc_seller_popup_button_background_hover_color', 'ht_single_product_asc_seller_popup_button_background_hover_color', 'C52323'),
    ...
    $table_name = $wpdb->prefix . "huge_it_catalog_product_params";
    $sql_update_catalog_5 = "INSERT INTO `$table_name` (`name`, `title`, `value`) VALUES
    ('ht_single_product_asc_to_seller_input_border_size', 'ht_single_product_asc_to_seller_input_border_size', '1'),
    ('ht_single_product_asc_to_seller_text_font_size', 'ht_single_product_asc_to_seller_text_font_size', '26'),
    ('ht_single_product_asc_to_seller_text_font_color', 'ht_single_product_asc_to_seller_text_font_color', 'D91A1A'),
    ('ht_single_product_asc_to_seller_input_border_color', 'ht_single_product_asc_to_seller_input_border_color', 'B3B3B3')";
    
    $query="SELECT name FROM ".$wpdb->prefix."huge_it_catalog_product_params";
    $update_catalog_5=$wpdb->get_results($query);
    if(end($update_catalog_5)->name=='ht_single_product_message_for_user'){
    ...
    //('ht_view5_allow_lightbox', 'Allow lightbox', 'Allow lightbox','on'),
    $table_name = $wpdb->prefix . "huge_it_catalog_params";
    $sql_update_catalog_6 = "INSERT INTO `$table_name` (`name`, `title`,`description`, `value`) VALUES
    ('ht_view5_allow_lightbox', 'Allow lightbox', 'Allow lightbox', 'on')";

    $query="SELECT name FROM ".$wpdb->prefix."huge_it_catalog_params";
    $update_catalog_6=$wpdb->get_results($query);
    if(end($update_catalog_6)->name=='htc_view3_pagination_position'){
            $wpdb->query($sql_update_catalog_6);
    };
/***add search ***/	
    $table_name = $wpdb->prefix . "huge_it_catalog_params";
    
    $sql_update_catalog_insert_search_options = "INSERT INTO `$table_name` (`name`, `title`,`description`, `value`) VALUES
    ('ht_view0_search_form_width', 'Search Form width', 'Search Form width', '94'),
    ('ht_view0_search_form_position', 'Search Form position', 'Search Form position', 'left'),		
    ('ht_view0_search_form_border_radius', 'Search Form Border Radius', 'Search Form Border Radius', '0'),	
    ('ht_view0_search_form_border_color', 'Search Form Border Color', 'Search Form Border Color', 'dedede'),	
    ('ht_view0_search_form_border_width', 'Search Form Border Width', 'Search Form Border Width', '1'),	
    ('ht_view0_search_button_background', 'Search Button Background Color', 'Search Button Background Color', '6EBFDC'),
    ('ht_view0_search_button_hover_color', 'Search Button Background Hover', 'Search Button Background Hover', '718B94'),
    ('ht_view0_search_button_text_color', 'Search Button Text Color', 'Search Button Text Color', 'ffffff'),
    ('ht_view1_search_form_width', 'Search Form width', 'Search Form width', '94'),
    ('ht_view1_search_form_position', 'Search Form position', 'Search Form position', 'left'),		
    ('ht_view1_search_form_border_radius', 'Search Form Border Radius', 'Search Form Border Radius', '0'),	
    ('ht_view1_search_form_border_color', 'Search Form Border Color', 'Search Form Border Color', 'dedede'),	
    ('ht_view1_search_form_border_width', 'Search Form Border Width', 'Search Form Border Width', '1'),	
    ('ht_view1_search_button_background', 'Search Button Background Color', 'Search Button Background Color', '6EBFDC'),
    ('ht_view1_search_button_hover_color', 'Search Button Background Hover', 'Search Button Background Hover', '718B94'),
    ('ht_view1_search_button_text_color', 'Search Button Text Color', 'Search Button Text Color', 'ffffff'),
    ('ht_view2_search_form_width', 'Search Form width', 'Search Form width', '94'),
    ('ht_view2_search_form_position', 'Search Form position', 'Search Form position', 'left'),		
    ('ht_view2_search_form_border_radius', 'Search Form Border Radius', 'Search Form Border Radius', '0'),	
    ('ht_view2_search_form_border_color', 'Search Form Border Color', 'Search Form Border Color', 'dedede'),	
    ('ht_view2_search_form_border_width', 'Search Form Border Width', 'Search Form Border Width', '1'),	
    ('ht_view2_search_button_background', 'Search Button Background Color', 'Search Button Background Color', '6EBFDC'),
    ('ht_view2_search_button_hover_color', 'Search Button Background Hover', 'Search Button Background Hover', '718B94'),
    ('ht_view2_search_button_text_color', 'Search Button Text Color', 'Search Button Text Color', 'ffffff'),
    ('ht_view3_search_form_width', 'Search Form width', 'Search Form width', '94'),
    ('ht_view3_search_form_position', 'Search Form position', 'Search Form position', 'left'),		
    ('ht_view3_search_form_border_radius', 'Search Form Border Radius', 'Search Form Border Radius', '0'),	
    ('ht_view3_search_form_border_color', 'Search Form Border Color', 'Search Form Border Color', 'dedede'),	
    ('ht_view3_search_form_border_width', 'Search Form Border Width', 'Search Form Border Width', '1'),	
    ('ht_view3_search_button_background', 'Search Button Background Color', 'Search Button Background Color', '6EBFDC'),
    ('ht_view3_search_button_hover_color', 'Search Button Background Hover', 'Search Button Background Hover', '718B94'),
    ('ht_view3_search_button_text_color', 'Search Button Text Color', 'Search Button Text Color', 'ffffff')";
	
    $query="SELECT name FROM ".$wpdb->prefix."huge_it_catalog_params";
    $update_catalog_insert_search_options=$wpdb->get_results($query);
    if(end($update_catalog_insert_search_options)->name=='ht_view5_allow_lightbox'){
            $wpdb->query($sql_update_catalog_insert_search_options);
    };	
/***add search xxx***/	
    
    $table_name = $wpdb->prefix . "huge_it_catalog_general_params";
    $sql_update_catalog_7 = "UPDATE `$table_name` SET value = 'Contact Seller' WHERE name='ht_single_product_asc_to_seller_text'";
    $wpdb->query($sql_update_catalog_7);
    
    //link_target1
    $table_name = $wpdb->prefix . "huge_it_catalogs";
    $sql_update_catalog_8 = "UPDATE `$table_name` SET link_target = 'on' WHERE id=8 or id=9";
    $wpdb->query($sql_update_catalog_8);
    
    
    //Button Position, Button Border Radius
    
    $table_name = $wpdb->prefix . "huge_it_catalog_product_params";
    $sql_update_catalog_9 = "INSERT INTO `$table_name` (`name`, `title`, `value`) VALUES
    ('ht_single_product_asc_seller_button_position', 'ht_single_product_asc_seller_button_position', 'left'),
    ('ht_single_product_asc_seller_button_border_radius', 'ht_single_product_asc_seller_button_border_radius', '5')";
    
    $query="SELECT name FROM ".$wpdb->prefix."huge_it_catalog_product_params";
    $update_catalog_9=$wpdb->get_results($query);
    if(end($update_catalog_9)->name=='ht_single_product_asc_to_seller_input_border_color'){
            $wpdb->query($sql_update_catalog_9);
    };
    ...
    $table_name = $wpdb->prefix . "huge_it_catalog_product_params";
    $sql_update_catalog_9 = "INSERT INTO `$table_name` (`name`, `title`, `value`) VALUES
    ('ht_single_product_comments_name_font_size', 'ht_single_product_comments_name_font_size', '18'),
    ('ht_single_product_comments_content_font_size', 'ht_single_product_comments_content_font_size', '14')";
    
    $query="SELECT name FROM ".$wpdb->prefix."huge_it_catalog_product_params";
    $update_catalog_9=$wpdb->get_results($query);
    if(end($update_catalog_9)->name=='ht_single_product_asc_seller_button_border_radius'){
            $wpdb->query($sql_update_catalog_9);
    };
    ...
    $table_name = $wpdb->prefix . "huge_it_catalog_params";
    $sql_update_catalog_10 = "INSERT INTO `$table_name` (`name`, `title`, `description`, `value`) VALUES
    ('ht_catalog_view0_price_text_font_color', 'Price Font Color', 'Price Font Color', 'FFFFFF'),
    ('ht_catalog_view1_price_text_font_color', 'Price Font Color', 'Price Font Color', 'FFFFFF'),
    ('ht_catalog_view2_price_text_font_color', 'Price Font Color', 'Price Font Color', 'FFFFFF'),
    ('ht_catalog_view3_price_text_font_color', 'Price Font Color', 'Price Font Color', 'FFFFFF'),
    ('ht_catalog_view5_price_text_font_color', 'Price Font Color', 'Price Font Color', 'FFFFFF')";
    
    $query="SELECT name FROM ".$wpdb->prefix."huge_it_catalog_params";
    $update_catalog_10=$wpdb->get_results($query);
    if(end($update_catalog_10)->name=='ht_view3_search_button_text_color'){
            $wpdb->query($sql_update_catalog_10);
    };
...
//        $wpdb->query("UPDATE ".$wpdb->prefix."huge_it_catalog_products SET single_product_url = 'http://huge-it.com/fields/order-website-maintenance/'");
    ...
    $needToUpdateSearchType = 0;
    $catalogProductsAllFieldsInArray = $wpdb->get_results("DESCRIBE " . $wpdb->prefix . "huge_it_catalogs", ARRAY_A);
    foreach($catalogProductsAllFieldsInArray as $catalogProductsFields){
        if ($catalogProductsFields['Field'] == 'search_by'){
            $needToUpdateSearchType = 1;
        }
    }
    if($needToUpdateSearchType == 0){
        $wpdb->query("ALTER TABLE ".$wpdb->prefix."huge_it_catalogs ADD search_by varchar(200)");
        $wpdb->query("UPDATE ".$wpdb->prefix."huge_it_catalogs SET search_by = '0,1'");
    }
    
    $table_name = $wpdb->prefix . "huge_it_catalog_general_params";
    $sql_update_catalog_11 = "INSERT INTO `$table_name` (`name`, `title`, `value`) VALUES
    ('ht_catalog_general_no_search_result_text', 'ht_catalog_general_no_search_result_text',  'No product matches your request.')";

    $query="SELECT name FROM ".$wpdb->prefix."huge_it_catalog_general_params";
    $update_catalog_11=$wpdb->get_results($query);
    if(end($update_catalog_11)->name=='ht_single_product_invalid_mail_text'){
            $wpdb->query($sql_update_catalog_11);
    };

    $table_name = $wpdb->prefix . "huge_it_catalog_params";
    $sql_update_catalog_12 = "INSERT INTO `$table_name` (`name`, `title`,`description`, `value`) VALUES
    ('ht_view0_image_behaviour', '', '', 'crop')";

    $query="SELECT name FROM ".$wpdb->prefix."huge_it_catalog_params";
    $update_catalog_12=$wpdb->get_results($query);
    if(end($update_catalog_12)->name=='ht_catalog_view5_price_text_font_color'){
        $wpdb->query($sql_update_catalog_12);
    };
