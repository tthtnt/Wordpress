...
( '".$post_id_1->post_title."', '".$row->id."', '".$descnohtmlnoq1."', '".$post_image ."', '".$posturl."', 'image', 'on', 'on', '2', '1' )";
...
function apply_cat($id)
{
            ...
            if(isset($_POST["content"])){
                ...
                if(isset($_POST['searchBy'])){
                    $searchBy = '';
                    $searchParams = $_POST['searchBy'];
                    foreach($searchParams as $searchParam){
                        $searchBy .=  $searchParam.',';
                    }
                    $searchBy = substr($searchBy,0,-1);
                }
                elseif(isset($_POST['catalog_search'])){
                    $searchBy = '';
                }
                $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_catalogs SET  search_by = '%s'  WHERE id = '%s' ", $searchBy , $id));
                //######################### Search Params End#################################//
