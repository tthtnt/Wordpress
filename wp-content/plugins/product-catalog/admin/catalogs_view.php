	...
	<div id="poststuff">
		<div id="catalogs-list-page">
			...
			$serch_fields='<div class="alignleft actions"">
				<label for="search_events_by_title" style="font-size:14px">'.__("Filter").': </label>
					<input type="text" name="search_events_by_title" value="'.$serch_value.'" id="search_events_by_title" onchange="clear_serch_texts()">
			</div>
			<div class="alignleft actions">
				<input type="button" value="Search" onclick="document.getElementById(\'page_number\').value=\'1\'; document.getElementById(\'serch_or_not\').value=\'search\';
				 document.getElementById(\'admin_form\').submit();" class="button-secondary action">
				 <input type="button" value="'.__("Reset").'" onclick="window.location.href=\'admin.php?page=catalogs_huge_it_catalog\'" class="button-secondary action">
			</div>';
...
jQuery(document).ready(function($){
	...
	jQuery('.huge-it-newuploader .button').click(function(e) {
		...
		custom_uploader = wp.media.frames.file_frame = wp.media({
			title: 'Inter Into Post',
			button: {
				text: 'Inter Into Post'
			},
			multiple: true
		});
				...
				<div id="post-body">
						...
						<li <?php if($j%2==0){echo "class='has-background'";}$j++; ?>>
							...
							<div class="image-options">
								<div class="options-container">
									...
									<div class="show_view">
										<label for="view_link_open_type<?php echo $rowimages->id; ?>"><?php echo __("Product page new tab","product-catalog");?></label>
									...
									<div>
										<label for="single_product_url_type<?php echo $rowimages->id; ?>"><?php echo __("Product Page Custom Link","product-catalog");?>:</label>
										<div style="position: relative;">
											<input class="text_area product_url_select" type="text" id="single_product_url_type<?php echo $rowimages->id; ?>" name="single_product_url_type<?php echo $rowimages->id; ?>" value="<?php echo esc_attr($rowimages->single_product_url_type); ?>">
									...
									<?php
											...
											foreach ($allParamsAndChildsInArray as $singleParamAndChild) {
												if($singleParamAndChild != "" && $singleParamAndChild != " "){
													...
													unset($separateParamAndChildWithoutParam[0]);     //there is a param field without param name
													$separateParamAndChildWithoutParam = array_reverse($separateParamAndChildWithoutParam);      //there is a ordered param field without param name
					...
					<?php } ?>
                        ...
                        <script>
                                    ...
                                    jQuery(document).on('keypress', '#add_cat_input input', function (e){
                                        if(jQuery(this).val() !== "") {
                                            if(e.keyCode == 13) {
                                    ...
                                    jQuery(document).on('change', '.firstParam', function (e){
                                        ...
                                        var params_input_old_val = jQuery(this).parent().parent().parent().parent().parent().find(".parameters").val();
                                        ...
                                        forEach( params_input_old_val_in_array, function( node ) {
                                            if (node != "") {
                                    ...
                                    jQuery(document).on('click', '#delete_cat', function (){
                                        ...
                                        var del_val_index = jQuery(this).parent().parent().index();
                                        var all_categories_old = jQuery('.inside #allCategories').val();
...
//                                      ###########     <<<<   allCategories hidden input changing     ###########                                       
                                        ...
                                        var newValue = "";
                                        var forEach = Function.prototype.call.bind( Array.prototype.forEach );
                                            forEach( all_categories_old_in_array, function( node ) {
...
//                                        ###########   >>>>     allCategories hidden input changing       ###########  
                                        ...
                                        jQuery('.parameters').each(function(){
                                            ...
                                            var all_params_old_val = jQuery(this).val();
                                            var all_params_old_val_in_array = all_params_old_val.split('*()*');
                                            if(all_params_old_val_in_array[0] == "" && need_to_much_index == 0){
                                                del_val_index = parseInt(del_val_index) + parseInt(1); need_to_much_index = 1;
                                            }
                                            var old_param_and_child = "*()*" + all_params_old_val_in_array[del_val_index];
                                            
                                            
                                            var foreach_key = 0;
                                            var all_params_new_val = "";
                                            var forEach = Function.prototype.call.bind( Array.prototype.forEach );
                                            forEach( all_params_old_val_in_array, function( node ) {
                                                if(foreach_key != del_val_index && node != ""){
                                                    if(all_params_new_val != "")
                                                        all_params_new_val = all_params_new_val + "*()*" +node;
                                                    else all_params_new_val = all_params_new_val + node;
                                                }
                                                
                                                foreach_key++;
                                            });
                                            jQuery(this).val(all_params_new_val);
			...
			<div id="postbox-container-1" class="postbox-container">
				<div id="side-sortables" class="meta-box-sortables ui-sortable">
					...
					<ul id="catalog-unique-options-list">
						...
						<li>
							<label for="catalog_search"><?php echo __( 'Item search ', 'product-catalog' );?></label>
						...
						<li class="multicheck">
							...
							<div>
								<span><input type="checkbox" name="searchBy[desc]" value="0" id="by_desc" <?php if($row->search_by != '' && in_array(0, explode(',',$row->search_by))){ echo 'checked="checked"'; } ?> />
								<label for="by_desc"><?php echo __( 'Description', 'product-catalog' );?></label></span>
								<span><input type="checkbox" name="searchBy[param]" value="1" id="by_parameter" <?php if(in_array(1, explode(',',$row->search_by))){ echo 'checked="checked"'; } ?> />
